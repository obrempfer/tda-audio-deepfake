...
FOLLOWUP_SET="${FOLLOWUP_SET:-gates}"
CACHE_NAMESPACE="${CACHE_NAMESPACE:-df_part1_${FOLLOWUP_SET}_shared}"
...
case "$FOLLOWUP_SET" in
  gates)
    declare -a RUNS=(
      "keep_low_gate_off|configs/experiments/ablation/cubical_best_band_keep_low_gate_off.yaml"
      "keep_low_gate10|configs/experiments/ablation/cubical_best_band_keep_low.yaml"
      "keep_low_gate12|configs/experiments/ablation/cubical_best_band_keep_low_gate12.yaml"
    )
    ;;
  keep_low_c)
    declare -a RUNS=(
      "keep_low_c2|configs/experiments/ablation/cubical_best_band_keep_low_c2.yaml"
      "keep_low_c4|configs/experiments/ablation/cubical_best_band_keep_low.yaml"
  ...
  keep_low_h1_c)
    declare -a RUNS=(
      ...
      "keep_low_h1_c4|configs/experiments/ablation/cubical_best_band_keep_low_h1_only.yaml"
      "keep_low_h1_c8|configs/experiments/ablation/cubical_best_band_keep_low_h1_only_c8.yaml"
    )
    ;;
  *)
...
RUN_TAG="${RUN_TAG:-df_part1_${FOLLOWUP_SET}_$(date +%Y%m%d_%H%M%S)}"
SUMMARY_LOG="$RESULTS_ROOT/logs/${RUN_TAG}_summary.log"

echo "Run tag: $RUN_TAG" | tee "$SUMMARY_LOG"
echo "Follow-up set: $FOLLOWUP_SET" | tee -a "$SUMMARY_LOG"
