...
CACHE_NAMESPACE="${CACHE_NAMESPACE:-morse_keep_low_norm_sweep_shared}"
...
GRAPH_K_VALUES="${GRAPH_K_VALUES:-4 8 12 16}"
MORSE_NORMALIZATION_VALUES="${MORSE_NORMALIZATION_VALUES:-none feature}"
SIMPLIFICATION="${SIMPLIFICATION:-difference}"
BASE_CONFIG="${BASE_CONFIG:-$ROOT_DIR/configs/experiments/ablation/morse_smale_best_band_keep_low.yaml}"
...
RUN_TAG="${RUN_TAG:-morse_keep_low_norm_sweep_$(date +%Y%m%d_%H%M%S)}"
...
echo "Morse normalization values: $MORSE_NORMALIZATION_VALUES" | tee -a "$SUMMARY_LOG"
echo "Simplification: $SIMPLIFICATION" | tee -a "$SUMMARY_LOG"

fail_count=0
run_count=0

for graph_k in $GRAPH_K_VALUES; do
  for morse_norm in $MORSE_NORMALIZATION_VALUES; do
    run_name="keep_low_k${graph_k}_norm_${morse_norm}"
...
cfg["morse_smale"]["normalization"] = None if "$morse_norm" == "none" else "$morse_norm"
cfg["morse_smale"]["simplification"] = "$SIMPLIFICATION"
...
PY
    ...
    echo "  morse_normalization=$morse_norm" | tee -a "$SUMMARY_LOG"
    echo "  simplification=$SIMPLIFICATION" | tee -a "$SUMMARY_LOG"
