...
CACHE_NAMESPACE="${CACHE_NAMESPACE:-mlaad_tiny_transfer_shared}"
...
mkdir -p "$RESULTS_ROOT/logs" "$SHARED_CACHE_DIR" "$RUNTIME_ROOT/mpl" "$RUNTIME_ROOT/pycache" "$RUNTIME_ROOT/xdg" "$MATERIALIZED_ROOT"
...
RUN_TAG="${RUN_TAG:-mlaad_tiny_transfer_$(date +%Y%m%d_%H%M%S)}"
...
EVAL_PROTOCOL="$materialized_dir/protocol.txt"
EVAL_AUDIO_DIR="$materialized_dir/audio"

declare -a RUNS=(
  "cubical_keep_low_gate12|configs/experiments/ablation/cubical_best_band_keep_low_gate12.yaml"
  "morse_keep_low_k4_norm_none|configs/experiments/ablation/morse_smale_best_band_keep_low_k4_norm_none.yaml"
)
...
echo "Materialized dir: $materialized_dir" | tee -a "$SUMMARY_LOG"
echo "Eval protocol: $EVAL_PROTOCOL" | tee -a "$SUMMARY_LOG"
echo "Eval audio dir: $EVAL_AUDIO_DIR" | tee -a "$SUMMARY_LOG"
echo "MLAAD languages: $MLAAD_LANGUAGES" | tee -a "$SUMMARY_LOG"
echo "MLAAD max per class: $MLAAD_MAX_PER_CLASS" | tee -a "$SUMMARY_LOG"
