...
PHASE="${PHASE:-smoke}"
CACHE_NAMESPACE="${CACHE_NAMESPACE:-df_part1_${PHASE}_shared}"
...
RUN_TAG="${RUN_TAG:-df_part1_${PHASE}_$(date +%Y%m%d_%H%M%S)}"
SUMMARY_LOG="$RESULTS_ROOT/logs/${RUN_TAG}_summary.log"

declare -a RUNS=(
  "full_reference|configs/experiments/cubical_mel_best_field_svm.yaml|data/results/holdout_all_full_20260418_154849_phaseA_holdout_reference/model.pkl"
  "keep_low|configs/experiments/ablation/cubical_best_band_keep_low.yaml|data/results/holdout_all_full_20260418_154849_phaseA_holdout_keep_low/model.pkl"
  "keep_low_h1|configs/experiments/ablation/cubical_best_band_keep_low_h1_only.yaml|data/results/holdout_all_full_20260418_154849_phaseB_holdout_keep_low_h1/model.pkl"
)

echo "Run tag: $RUN_TAG" | tee "$SUMMARY_LOG"
echo "Phase: $PHASE" | tee -a "$SUMMARY_LOG"
